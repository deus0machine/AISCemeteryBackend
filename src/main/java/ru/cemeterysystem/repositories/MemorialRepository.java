...
public interface MemorialRepository extends JpaRepository<Memorial, Long> {
    ...
    @Query(value = "SELECT * FROM memorials m WHERE " +
           ...
           "LOWER(m.middle_name) LIKE LOWER(CONCAT('%', :query, '%')) OR " +
           "LOWER(CONCAT(m.last_name, ' ', m.first_name)) LIKE LOWER(CONCAT('%', :query, '%')) OR " +
           "LOWER(CONCAT(m.first_name, ' ', m.last_name)) LIKE LOWER(CONCAT('%', :query, '%')) OR " +
           "LOWER(CONCAT(m.last_name, ' ', m.first_name, ' ', COALESCE(m.middle_name, ''))) LIKE LOWER(CONCAT('%', :query, '%')) OR " +
           "LOWER(CONCAT(m.first_name, ' ', COALESCE(m.middle_name, ''), ' ', m.last_name)) LIKE LOWER(CONCAT('%', :query, '%')) OR " +
           "LOWER(CONCAT(COALESCE(m.middle_name, ''), ' ', m.last_name, ' ', m.first_name)) LIKE LOWER(CONCAT('%', :query, '%'))) AND " +
           "(:location IS NULL OR LOWER(m.main_address) LIKE LOWER(CONCAT('%', :location, '%')) OR " +
           "LOWER(m.burial_address) LIKE LOWER(CONCAT('%', :location, '%'))) AND " +
           "(:startDate IS NULL OR m.birth_date >= CAST(:startDate AS date)) AND " +
           "(:endDate IS NULL OR m.death_date <= CAST(:endDate AS date)) AND " +
           "(:isPublic IS NULL OR m.is_public = :isPublic) AND " +
           "m.is_blocked = false AND " +
           "m.publication_status = 'PUBLISHED'", nativeQuery = true)
    ...
    @Query(value = "SELECT * FROM memorials m WHERE " +
           "(:query IS NULL OR :query = '' OR " +
           "LOWER(m.fio) LIKE LOWER(CONCAT('%', :query, '%')) OR " +
           "LOWER(CONCAT(m.last_name, ' ', m.first_name, ' ', COALESCE(m.middle_name, ''))) LIKE LOWER(CONCAT('%', :query, '%'))) AND " +
           "(:firstName IS NULL OR :firstName = '' OR LOWER(m.first_name) LIKE LOWER(CONCAT('%', :firstName, '%'))) AND " +
           "(:lastName IS NULL OR :lastName = '' OR LOWER(m.last_name) LIKE LOWER(CONCAT('%', :lastName, '%'))) AND " +
           "(:middleName IS NULL OR :middleName = '' OR LOWER(m.middle_name) LIKE LOWER(CONCAT('%', :middleName, '%'))) AND " +
           "(:birthDateFrom IS NULL OR m.birth_date >= CAST(:birthDateFrom AS date)) AND " +
           "(:birthDateTo IS NULL OR m.birth_date <= CAST(:birthDateTo AS date)) AND " +
           "(:deathDateFrom IS NULL OR m.death_date >= CAST(:deathDateFrom AS date)) AND " +
           "(:deathDateTo IS NULL OR m.death_date <= CAST(:deathDateTo AS date)) AND " +
           "(:location IS NULL OR :location = '' OR " +
           "LOWER(m.main_address) LIKE LOWER(CONCAT('%', :location, '%')) OR " +
           "LOWER(m.burial_address) LIKE LOWER(CONCAT('%', :location, '%'))) AND " +
           "(:isPublic IS NULL OR m.is_public = :isPublic) AND " +
           "m.is_blocked = false AND " +
           "m.publication_status = 'PUBLISHED' " +
           "ORDER BY " +
           "CASE WHEN :sortBy = 'lastName' AND :sortDirection = 'asc' THEN m.last_name END ASC, " +
           "CASE WHEN :sortBy = 'lastName' AND :sortDirection = 'desc' THEN m.last_name END DESC, " +
           "CASE WHEN :sortBy = 'firstName' AND :sortDirection = 'asc' THEN m.first_name END ASC, " +
           "CASE WHEN :sortBy = 'firstName' AND :sortDirection = 'desc' THEN m.first_name END DESC, " +
           "CASE WHEN :sortBy = 'birthDate' AND :sortDirection = 'asc' THEN m.birth_date END ASC, " +
           "CASE WHEN :sortBy = 'birthDate' AND :sortDirection = 'desc' THEN m.birth_date END DESC, " +
           "CASE WHEN :sortBy = 'deathDate' AND :sortDirection = 'asc' THEN m.death_date END ASC, " +
           "CASE WHEN :sortBy = 'deathDate' AND :sortDirection = 'desc' THEN m.death_date END DESC, " +
           "CASE WHEN :sortBy = 'createdAt' AND :sortDirection = 'asc' THEN m.created_at END ASC, " +
           "CASE WHEN :sortBy = 'createdAt' AND :sortDirection = 'desc' THEN m.created_at END DESC, " +
           "m.last_name ASC", nativeQuery = true)
